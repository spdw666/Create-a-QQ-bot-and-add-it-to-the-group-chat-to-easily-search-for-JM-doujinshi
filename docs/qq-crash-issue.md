...
## 已尝试的方案 Attempts（含最终修复）

| 方案 | 结果 | 原因 |
|------|------|------|
| watchdog 自动拉起 | ✅ **曾是问题本身，v6 已修复** | v2-v5 用裸 `ss` 检测（crontab PATH 无 /usr/sbin）→ 永远误判 down → 冷却结束误杀 QQ；v6 改 `/usr/sbin/ss` 绝对路径 |
| 每日 04:30 主动重启 QQ + 增加 2GB swap | ⚠️ 保留（无害） | 对误杀问题无效；每日一次重启本身无害 |
...
| o3HookMode=0（社区假死修复） | 未试 | 不必要（真相已明） |
| **NapCat 反检测 bypass 六项**（hook/window/module/process/container/js） | ✅ 生效中（应对真掉线：风控踢号） | 需改**账号级配置** `napcat_<QQ号>.json`（主配置会被覆盖）；开启后 `-q` 快速登录成功、token 不再被清 |
...
## 2026-08-14 晚进展 Update（19:15 记录，20:20 部分修正）

1. **腾讯风控实锤**：18:41 用户收到 QQ 安全中心通知"已登录设备风险提醒"——设备 ser678257072092（本服务器）被判"**设备存外挂或其他软件影响QQ正常使用**"，平台已对风险设备下线处理（最后登录 15:45:15）。~~此前"崩溃间隔缩短"是客户端崩溃 + 风控踢号叠加~~（20:20 修正：间隔缩短 = watchdog 误杀冷却调整，见最终结论）；`-q` 快速登录 token 频繁失效（风控清 token）得到解释。
2. **反检测开启**：NapCat 自带 bypass 六项反检测（默认全关），已开启并验证生效（启动日志确认加载 true 版）。开启后 `-q` 免扫码登录成功，token 不再被清。
3. 若仍有风控踢号，下一步候选：清理 `~/.config/QQ` 风控数据（GitHub Discussion #1888，效果因人而异）、SnowLuma 框架、OpenShamrock 云手机。

## 假设原因 Hypotheses（20:20 重新评估）

1. ~~腾讯对旧协议端的风控施压导致间隔缩短~~ → **推翻**：间隔 30→20→10 分钟 = watchdog 冷却时间三次调整记录，与风控无关。
2. **QQ 3.2.30 客户端 bug（存疑，非周期主因）**：Bugly 崩溃日志确实出现过（18:36 前后有真崩记录），但不构成"10 分钟周期"——周期是 watchdog 误杀制造的；QQ 3.2.30 在无干扰下可稳定运行（v6 修复后已存活远超旧"周期"）。
3. **账号/设备风控（已实锤 ✅，仍在应对）**：2026-08-14 18:41 收到腾讯官方风险设备通知，服务器设备被判"外挂影响QQ正常使用"并主动下线——NapCat 注入特征被腾讯检测。反检测 bypass 已开启应对。

## 求助 Help wanted 🙏（已自我解决，留档）

~~如果你遇到过同样问题并解决了，或者知道根因，请在 Issue 里回复，欢迎 PR~~：

1. ~~有没有已知的 QQ 3.2.30-50969 崩溃 bug 及官方修复版本号？~~ → **问题本身不存在**：周期掉线是 watchdog 误杀；QQ 3.2.30 无干扰下运行正常
2. ~~腾讯最新 Linux QQ 的下载源？~~ → 已找到官方直链（qqdl.gtimg.cn，3.2.32-52194），但 NapCat 4.18.18 尚未适配，暂不可用
3. ~~SnowLuma 或其他框架是否根治过同类问题？~~ → 已不需要（误杀修复后待观察；如风控仍频繁再评估）
