...
代码、虚拟环境和 SQLite 数据库通常很小，可以继续放在 C 盘；漫画页面、ZIP 和分享文件则可能很大。可在本机 `.env` 设置绝对路径，把大文件单独放到空间更充足的盘：

```dotenv
JM_DOWNLOAD_DIR=D:\JM-Niang-data\downloads
JM_SHARE_DIR=D:\JM-Niang-data\http_dl
```

重启机器人后，新下载和 HTTP 分享文件会使用这些目录，`自查` 与管理员磁盘诊断也会改为显示下载目录所在磁盘的可用空间。未设置时，程序仍使用项目内的 `downloads/`、`http_dl/`，保持新电脑的一键启动兼容。迁移已有缓存时，先停止 JM娘，再移动目录内容，最后启动并发送 `自查` 验证。
